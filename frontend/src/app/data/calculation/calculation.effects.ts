...
export class CalculationEffects {
  ...
  loadCalculationResult$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CalculationActions.loadCalculationResult),
      mergeMap(({ calculationId }) =>
        this.calcService
          .addResult(calculationId)
          .then(() => CalculationActions.loadCalculationResultSuccess({ calculationId }))
      )
    )
  );
  ...
  deleteCalculation$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        from(
          this.calcService.deleteResults(
            action.type === CalculationActions.deleteCalculation.type
              ? [action.calculationToBeDeleted.id]
              : action.calculationIds
          )
        ).pipe(
          mergeMap(() =>
            of(
              CalculationActions.deleteCalculationSuccess(), // does nothing atm
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ maxValue, comparisonTitle }) =>
        this.calcService.getComparisonLegend(maxValue).pipe(
          map((legend) =>
            CalculationActions.fetchComparisonLegendSuccess({ legend, comparisonTitle, maxValue })
          ),
          catchError(({ status, error: message }) =>
            of(
              CalculationActions.fetchLegendFailure({
  ...
  removeBatchProcess$ = createEffect(() =>
    this.actions$.pipe(
  ...
  cancelBatchProcess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ id }) =>
        this.calcService
          .cancelBatchProcess(id)
          .pipe(map(() => CalculationActions.cancelBatchProcessSuccess({ id })))
      )
    )
  );

  generateCompoundComparison$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([{ comparisonName, calculationIds }, baseline]) =>
        this.calcService.generateCompoundComparison(comparisonName, calculationIds, baseline).pipe(
          mergeMap((comparisonId) =>
            of(
              CalculationActions.generateCompoundComparisonSuccess({ comparisonId }),
  ...
  deleteMultipleCompoundComparisons$ = createEffect(() =>
    this.actions$.pipe(
  ...
  $calculateActiveScenario = createEffect(() =>
    this.actions$.pipe(
