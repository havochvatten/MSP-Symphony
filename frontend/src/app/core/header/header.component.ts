...
export class HeaderComponent implements OnInit {
  ...
  logOutUserMenu?: MenuItem[];
  logInUserMenu?: MenuItem[];
  ...
  ngOnInit() {
    ...
    ).subscribe(baselines => this.buildUserMenus(baselines));
  }

  private buildUserMenus(baselines: Baseline[]) {
    ...
    if (environment.externManual) {
      this.logOutUserMenu.push({ name: 'user-menu.support', icon: gmHelpCircle, click: this.openManual });
    }
    this.logOutUserMenu.push(...changeLanguageMenuItem);
    if (baselines.length > 1) {
      this.logOutUserMenu.push({ name: 'user-menu.change-baseline', icon: faLayerGroup, click: () => this.changeBaseline() });
    }
    this.logOutUserMenu.push(...aboutMenuItems);
    this.logOutUserMenu.push(...logOutUserMenuItem);
  ...
  login = () => {
    this.store.dispatch(UserActions.navigateTo({ url: '/login' }));
  };
