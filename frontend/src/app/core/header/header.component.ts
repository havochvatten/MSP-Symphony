...
export class HeaderComponent implements OnInit {
  ...
  private buildUserMenus(baselines: Baseline[]) {
    ...
    if (baselines.length > 1) {
      this.logOutUserMenu.push({
        name: 'user-menu.change-baseline',
        icon: faLayerGroup,
        click: () => this.changeBaseline()
      });
    }
    this.logOutUserMenu.push(...aboutMenuItems, ...logOutUserMenuItem);
    this.publicMenu = [...changeLanguageMenuItem, ...logInUserMenuItem];
  }
